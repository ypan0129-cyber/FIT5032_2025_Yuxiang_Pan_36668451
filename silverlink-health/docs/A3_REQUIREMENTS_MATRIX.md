...
| D.4 Public cloud deployment | The complete application, SPA routes, Firestore rules and serverless functions are available at a public production URL. | Verified | Stage 11 | Firebase Hosting serves `dist` on both default domains with SPA fallback, production CSP/security headers, safe PWA caching, target-limited deployment scripts and `docs/DEPLOYMENT.md`; published Firestore Rules and the Alibaba Cloud serverless API complete the cross-cloud production architecture without requiring Blaze | 75 frontend, 37 function and 6 Firestore Rules tests passed with both production dependency audits at zero vulnerabilities; Firebase deployed 20 Hosting files and Rules; both domains and six representative routes returned 200, security/cache headers passed, Alibaba CORS returned 204 for both origins, unauthenticated POST remained 401, and the public API exposed six allowlisted records without private fields; desktop/mobile browser checks had no console error or overflow; a real member then verified Google sign-in, rating save, support-plan email/PDF delivery and saved-resource reload in public-origin Offline mode |
