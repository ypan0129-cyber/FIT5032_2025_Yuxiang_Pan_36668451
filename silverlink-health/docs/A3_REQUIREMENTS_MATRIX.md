...
| D.4 Public cloud deployment | The complete application, SPA routes, Firestore rules and serverless functions are available at a public production URL. | Deployed; real-account authentication and offline-reload acceptance remain before final verification. | Stage 11 | Firebase Hosting serves `dist` on both default domains with SPA fallback, production CSP/security headers, safe PWA caching, target-limited deployment scripts and `docs/DEPLOYMENT.md`; published Firestore Rules and the Alibaba Cloud serverless API complete the cross-cloud production architecture without requiring Blaze | 75 frontend, 37 function and 6 Firestore Rules tests passed with both production dependency audits at zero vulnerabilities; Firebase deployed 20 Hosting files and Rules; both domains and six representative routes returned 200, security/cache headers passed, Alibaba CORS returned 204 for both origins, unauthenticated POST remained 401, the public API exposed six allowlisted records without private fields, and desktop/mobile browser checks had no console error or overflow; real-account Google sign-in, support-plan delivery and public-origin offline reload remain manual |
...
| E.3 WCAG 2.1 AA accessibility | All completed A3 workflows are audited against WCAG 2.1 AA with automated and documented manual checks. | Verified | Stage 10 | Route title and focus management, keyboard skip/mobile navigation, accessible table/map/chart alternatives, high-contrast focus, reduced-motion/forced-colors support, 5 focused accessibility tests and `ACCESSIBILITY.md` | 71 frontend tests, 37 function tests, production build and zero-vulnerability audits passed; production-preview checks verified keyboard focus, Skip-link activation, mobile-menu Escape, accessibility-tree structure, external-link announcements, four workflows without horizontal overflow at 390 x 844, media preferences and an error-free console |
...
| Offline support | The application reports connectivity state and lets users open previously saved public resources while offline. | Verified | Stage 9 | Installable PWA manifest and Workbox service worker, persistent connectivity status, allowlisted public-ID local storage, public saved directory and details, offline fallback, disabled live actions and network-route guards | 66 frontend tests, 37 function tests, production build, zero-vulnerability production audits and `git diff --check` passed; browser checks saved two resources, stopped the preview server, reloaded `/saved` from Service Worker responses, opened a saved detail offline, disabled ratings and websites, redirected a network-backed route, recovered online actions and produced no console errors; cache inspection found exactly 18 same-origin static assets with no API/Firebase/Alibaba URLs, and the 390 x 844 layout had no page overflow or clipped controls |
