...
| Offline support | The application reports connectivity state and lets users open previously saved public resources while offline. | Implemented | Stage 9 | Installable PWA manifest and Workbox service worker, persistent connectivity status, allowlisted public-ID local storage, public saved directory and details, offline fallback, disabled live actions and network-route guards | 66 frontend tests, 37 function tests, production build, zero-vulnerability production audits and `git diff --check` passed; generated service worker contains 18 unique static URLs, no API/Firebase/Alibaba URLs, no runtime caches and the protected-route denylist; browser offline/reconnect, cache inspection and 390 x 844 checks pending |
