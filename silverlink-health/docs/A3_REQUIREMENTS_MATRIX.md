...
| E.4 Data export | Users can export the current filtered and sorted table data without leaking internal identifiers. | Planned | Stage 3 | CSV and PDF export services and table export controls | Content tests for escaping, column selection, active filters and exported-file readability |
...
| --- | --- | --- | --- | --- | --- |
| Firestore analytics | Interactive charts display privacy-safe rating aggregates sourced from Firestore and include an equivalent data table. | Planned | Stage 6 | Aggregate documents, rating trigger/backfill path, Firestore rules and interactive chart | Aggregate/trigger tests, rule tests and chart interaction/accessibility checks |
| Administrator dashboard | A separate `admin` role can view system-level user, role, rating and email metrics through server-authorised operations. | Planned | Stage 7 | Admin route, navigation, callable metrics function and server-side role enforcement | Member/staff denial checks and successful admin workflow in the configured project |
| Public REST API | At least two versioned, read-only endpoints expose resources and rating summaries without personal data. | Planned | Stage 8 | HTTP function, `/api/v1/resources`, `/api/v1/resources/:id/summary`, CORS and OpenAPI documentation | Endpoint contract, validation, error, caching and production URL checks |
