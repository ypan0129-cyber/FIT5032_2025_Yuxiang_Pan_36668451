...
| Administrator dashboard | A separate `admin` role can view system-level user, role, rating and email metrics through server-authorised operations. | Implemented locally; cloud verification pending | Stage 7 | Protected `/admin` route and navigation, server-authorised `POST /admin/metrics`, field-limited aggregate queries, admin-only role enforcement and responsive dashboard states | 55 frontend tests, 30 function tests, 6 Firestore emulator tests, Node.js 20 execution, production build and zero-vulnerability production audits passed; updated function deployment, configured-project role assignment and desktop/mobile browser checks remain pending |
