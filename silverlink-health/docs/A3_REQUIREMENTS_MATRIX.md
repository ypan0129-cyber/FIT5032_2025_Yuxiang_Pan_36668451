...
| --- | --- | --- | --- | --- | --- |
| Firestore analytics | Interactive charts display privacy-safe rating aggregates sourced from Firestore and include an equivalent data table. | Implemented locally; cloud verification pending | Stage 6 | Server-authoritative transactional rating writes, `ratingAnalytics` documents, staff rebuild operation, private-rating Firestore rules, Chart.js segmented chart and searchable/exportable table equivalent | 49 frontend tests, 25 function tests, 5 Firestore emulator tests, production build and zero-vulnerability production audits passed; updated function/rules deployment, legacy rebuild and desktop/mobile browser checks remain pending |
