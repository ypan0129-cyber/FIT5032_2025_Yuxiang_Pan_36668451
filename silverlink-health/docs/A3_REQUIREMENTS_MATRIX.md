...
| D.4 Public cloud deployment | The complete application, SPA routes, Firestore rules and serverless functions are available at a public production URL. | Implemented locally; Firebase Hosting deployment and public-origin acceptance checks are pending. | Stage 11 | Firebase Hosting serves `dist` with SPA fallback, production CSP/security headers, safe PWA caching, local Hosting emulator, target-limited deployment scripts, 4 focused tests and `docs/DEPLOYMENT.md`; the serverless API remains on the deployed Alibaba Cloud endpoint so Spark does not require Firebase Functions | 75 frontend tests, production build, zero-vulnerability audit and `git diff --check` passed; the Hosting emulator returned 200 for a deep resource URL, no-store HTML/PWA headers, immutable hashed assets and the expected production security headers; public Firebase URLs and cross-cloud workflows still require deployment verification |
