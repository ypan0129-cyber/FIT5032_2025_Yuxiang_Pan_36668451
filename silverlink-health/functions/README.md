# Support plan function

This package runs the support-plan email workflow as an Alibaba Cloud Function
Compute event function. The HTTP trigger is public at the cloud layer because
browser clients cannot create Alibaba Cloud request signatures. The function
still requires and verifies a Firebase ID token on every `POST` request.
